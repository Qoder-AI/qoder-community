...
name: dlazy-sora-2
version: 1.0.0
description: 使用强大的 Sora 2 模型，根据文字提示或参考图生成极其逼真且符合物理规律的高清视频。
metadata: {"clawdbot":{"emoji":"🤖","requires":{"bins":["npm","npx"]},"install":"npm install -g @dlazy/cli"},"openclaw":{"systemPrompt":"当调用此技能时，可以使用 dlazy sora-2 -h 查看帮助信息。"}}
---

# dlazy-sora-2

使用强大的 Sora 2 模型，根据文字提示或参考图生成极其逼真且符合物理规律的高清视频。
...
```bash
# 基础调用：
dlazy sora-2 --prompt '提示词内容' --image '/path/to/image.png'

# 复杂调用：
dlazy sora-2 --prompt '提示词内容' --image 'https://oss.dlazy.com/image.png'
```
